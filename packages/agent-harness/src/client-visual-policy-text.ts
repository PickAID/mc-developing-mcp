export const CLIENT_VISUAL_CAPABILITY_POLICY_TEXT = [
  ...
  "For UI, rendering, and shader work, gather source, asset, archive, docs, and API-proof evidence together before choosing an implementation pattern.",
  "Keep screen layout/widgets, scalable UI asset semantics, render pipeline state, buffer/render type ownership, shader or post-processing chain, and asset references as separate evidence links.",
  "For external shader references, prefer the configured API key when activated; if no key is available, use a local Chrome DevTools or Playwright browser fallback and return only compact formula summaries mapped to Minecraft uniforms, samplers, render targets, reload lifecycle, and fallback evidence.",
