...
function findRequestedLineRange(input: {
  requestText: string;
  relativePath: string;
  fileName: string;
}): LineRangeEvidenceOptions {
  for (const hint of extractLineHints(input.requestText)) {
    if (
      hint.path === input.fileName ||
      hint.path === input.relativePath ||
      input.relativePath.endsWith(`/${hint.path}`)
    ) {
      return hint.endLine
        ? {
            startLine: hint.line,
...
function extractLineHints(
  ...
  const matches = requestText.matchAll(
    /\b([A-Za-z0-9_.$/-]+\.(?:java|gradle|kts|toml|properties)):(\d+)(?:-(\d+)|:\d+)?/gi
  );
