...
describe("source.bundle client visual shader references", () => {
  ...
  it("reports browser fallback when shader references are enabled without a key", async () => {
  ...
  it("routes browser fallback provider summaries through client visual evidence", async () => {
    const result = await executeClientVisualRequest({
      externalShaderReference: {
        enabled: true,
        credentialProvider: () => undefined,
        browserFallbackProvider: async (input) => [
          {
            title: `Fallback for ${input.browserFallback.query}`,
            shaderId: "browser1",
            formulaTerms: ["uv", "palette"],
            minecraftMapping: {
    ...
    expect(result.payload).toMatchObject({
      clientVisualEvidence: {
        externalShaderReference: {
          ...
          summaries: [
            {
              title: expect.stringContaining("Fallback for"),
              shaderId: "browser1"
            }
          ]
        }
