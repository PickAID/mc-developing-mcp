...
describe("mod archive batch reads", () => {
  ...
  it("returns bounded source metadata for selected Java entries", async () => {
    const workspaceRoot = await createWorkspace();
    const input = await createExecutorInput(
      workspaceRoot,
      "source.read com/example/content/Widget.java:20-100 from mods/content-mod.jar."
    );

    await expect(executeMcpServerModArchiveContent(input)).resolves.toMatchObject({
      matched: true,
      summary: "Read com/example/content/Widget.java from selected mod archive.",
      payload: {
        source: "mod_archive_content",
        mode: "read",
        requestedPath: "com/example/content/Widget.java",
        entry: {
          relativePath: "com/example/content/Widget.java",
          domain: "java"
        },
        startLine: 20,
        endLine: 60,
        totalLines: 62,
        truncated: true,
        nextReads: ["source.read com/example/content/Widget.java:20-60"],
        content: expect.stringContaining("  // line 60")
      }
    });
  });

  it("returns line metadata for explicit selected JSON line ranges", async () => {
    const workspaceRoot = await createWorkspace();
    const input = await createExecutorInput(
      workspaceRoot,
      "source.read data/demo/recipes/long_gear.json:2-4 from mods/content-mod.jar."
    );

    await expect(executeMcpServerModArchiveContent(input)).resolves.toMatchObject({
      matched: true,
      summary: "Read data/demo/recipes/long_gear.json from selected mod archive.",
      payload: {
        source: "mod_archive_content",
        mode: "read",
        requestedPath: "data/demo/recipes/long_gear.json",
        entry: {
          relativePath: "data/demo/recipes/long_gear.json",
          domain: "data"
        },
        startLine: 2,
        endLine: 4,
        totalLines: 5,
        truncated: true,
        nextReads: ["source.read data/demo/recipes/long_gear.json:2-4"],
        content: [
          "  \"type\": \"minecraft:crafting_shaped\",",
          "  \"pattern\": [\"###\"],",
          "  \"result\": \"demo:long_gear\""
        ].join("\n")
      }
    });
  });

  it("bounds selected Java entries without line hints", async () => {
    const workspaceRoot = await createWorkspace();
    const input = await createExecutorInput(
      workspaceRoot,
      "Read com/example/content/Widget.java from mods/content-mod.jar."
    );
    ...
    await expect(executeMcpServerModArchiveContent(input)).resolves.toMatchObject({
      payload: {
        ...
        totalLines: 62,
        truncated: true,
        nextReads: ["source.read com/example/content/Widget.java:1-41"],
        content: expect.not.stringContaining("  // line 60")
      }
    });
  });

  it("accepts filename-only line hints for selected Java entries", async () => {
    const workspaceRoot = await createWorkspace();
    const input = await createExecutorInput(
      workspaceRoot,
      "Read com/example/content/Widget.java around Widget.java:30:1 from mods/content-mod.jar."
    );
    ...
    await expect(executeMcpServerModArchiveContent(input)).resolves.toMatchObject({
      payload: {
        ...
        totalLines: 62,
        truncated: true,
        nextReads: ["source.read com/example/content/Widget.java:10-50"],
...
async function createWorkspace(): Promise<string> {
  ...
  await writeFile(
    ...
    createZip([
      ...
      {
        name: "data/demo/recipes/long_gear.json",
        content: [
          "{",
          "  \"type\": \"minecraft:crafting_shaped\",",
          "  \"pattern\": [\"###\"],",
      ...
      {
        name: "com/example/content/Widget.java",
