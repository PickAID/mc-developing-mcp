...
export async function readSelectedNestedEntry(input: {
  ...
  const result = applyModArchiveSourceLineRange(
    await readNestedArchiveContentFile({
    ...
    input.requestText,
    input.request.embeddedArchivePath
  );
  const payload = {
  ...
  for (const group of groupRequestsByEmbeddedArchive(input.requests)) {
    ...
    files.push(
      ...result.files.map((file) => ({
        ...
        ...applyModArchiveSourceLineRange(
          file,
          input.requestText,
          result.embeddedArchivePath
        )
      }))
    );
  }
