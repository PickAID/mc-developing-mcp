...
function extractTraceableAssetPaths(requestText?: string): string[] {
  ...
  const matches = requestText.matchAll(
    /\bassets\/[A-Za-z0-9_.-]+\/(?:atlases|blockstates|font|items|models|particles)\/[A-Za-z0-9_./+$-]+\.json\b/g
  );
...
function extractNestedTraceableAssetPaths(
  ...
  const matches = requestText.matchAll(
    new RegExp(
      `\\b${escapedArchivePath}!/(assets/[A-Za-z0-9_.-]+/(?:atlases|blockstates|font|items|models|particles)/[A-Za-z0-9_./+$-]+\\.json)\\b`,
      "g"
    )
  );
