...
- Latest implementation state before this progress update: clean at `8a00b94`
- Public MCP surface: one tool, `mc_develop`
- Latest full verification: `pnpm test` passed with 97 test files and 300 tests
...
- current progress update: loose resource reference tracing for blockstate/model/texture evidence
...
- Harness route hardening for assets-only resource-pack folders without `pack.mcmeta`.
...
- assets-only resource-pack route detection without relying on `pack.mcmeta`
...
4. Harden resource-pack-only workspace routing so loose `assets/**` can trigger resource evidence without `pack.mcmeta`.
