...
- `mc_develop` now exposes local `mdm-sources` release acceptance state through
  ...
  acceptance.
- `mdm-sources` release CI now builds through
  `tools/write-release-acceptance-report.mjs` before publishing. The workflow
...
- `mdm-sources` now has a single live release acceptance command:
  `tools/verify-live-release.mjs <manifest-path-or-url>`. It combines remote
...
- `mdm-sources` release workflow now writes GitHub Release notes from
  ...
  are uploaded.
- `mdm-sources` release workflow now runs post-publish live verification against
  the actual GitHub Release manifest URL. After create/upload, CI calls
  `tools/verify-live-release.mjs` against
